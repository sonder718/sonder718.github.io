...
def cal_orth(x,y):
    ...
    for i in range(x.shape[0]):#对于每个batch
        ...
        # #判断vertical_x_y和x[i]是否正交
        # proj_matmul_1=torch.zeros(200)   
        # proj_matmul_2=torch.zeros(200)   
        # proj_matmul_3=torch.zeros(200)   

        # for j in range(100):
        #     proj_matmul_1[i] = torch.matmul(x[i][j], res[i][j].t())#第i个batch的第j个点的特征向量和正交向量的点积
        #     proj_matmul_2[i]=torch.matmul(y[i][j], res[i][j].t())
        #     proj_matmul_3[i]=torch.matmul(x[i][j], y[i][j].t())

        #计算proj的最大值
        # max_num=torch.max(proj_matmul_1)
...
class hybrid_block(nn.Module):
    def __init__(self,channel,head):
        ...
        self.attention_block_down_self = attention_propagantion(channel, head)
        self.attention_block_up_self = attention_propagantion(channel, head)
        self.cat_filter_self=nn.Sequential(nn.Conv1d(2*channel,2*channel, kernel_size=1), nn.SyncBatchNorm(2*channel), nn.ReLU(),
                                      nn.Conv1d(2*channel, channel, kernel_size=1))
    ...
    #self-attention
        ...
        #所有点进行自注意力聚合
        desc1_all, desc2_all = self.attention_block_self(desc1, desc1), self.attention_block_self(desc2, desc2)
        
        #种子点和非种子点进行自注意力聚合
            #从本图所有点更新种子点
        cluster1, cluster2 = self.attention_block_down_self(cluster1, desc1), self.attention_block_down_self(cluster2, desc2)
            #从种子点更新所有点
        seed_weight=self.confidence_filter_self(torch.cat([cluster1,cluster2],dim=1))
        seed_weight=torch.sigmoid(seed_weight).squeeze(1)
        desc1_seed,desc2_seed=self.attention_block_up_self(desc1,cluster1,seed_weight),self.attention_block_up_self(desc2,cluster2,seed_weight)
        #正交化
